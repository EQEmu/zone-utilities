...
bool Map::Write(std::string filename) {
    ...
    while(model_iter != map_models.end()) {
        ...
        for(auto& set : textureSet) {
            eqLogMessage(LogTrace,
                         "Texture set for model %s with flag %u",
                         model_iter->second->GetName().c_str(),
                         set->GetFlags());
        }
    ...
    uint32_t out_size = eqemu::core::deflate_data(
        (std::byte*)ss.str().c_str(), (uint32_t)ss.str().length(), (std::byte*)&buffer[0], buffer_len);
    if(fwrite(&out_size, sizeof(uint32_t), 1, f) != 1) {
        eqLogMessage(
            LogError, "Failed to write %s because the compressed size header could not be written.", filename.c_str());
        fclose(f);
        return false;
    }
    ...
    if(fwrite(&uncompressed_size, sizeof(uint32_t), 1, f) != 1) {
        eqLogMessage(LogError,
                     "Failed to write %s because the uncompressed size header could not be written.",
                     filename.c_str());
        fclose(f);
        return false;
    }
    ...
    if(fwrite(&buffer[0], out_size, 1, f) != 1) {
        eqLogMessage(
            LogError, "Failed to write %s because the compressed data could not be written.", filename.c_str());
        fclose(f);
        return false;
    }
...
bool Map::CompileS3D(std::vector<EQEmu::S3D::WLDFragment>& zone_frags,
    ...
    std::vector<std::pair<std::shared_ptr<EQEmu::Placeable>, std::shared_ptr<EQEmu::S3D::SkeletonTrack>>>
        placables_skeleton;
                    ...
                    if(mod_ref->GetName().compare(plac->GetName()) == 0) {
                        ...
                        for(uint32_t m = 0; m < frag_refs.size(); ++m) {
                            if(object_frags[frag_refs[m] - 1].type == 0x2D) {
                                EQEmu::S3D::WLDFragment2D& r_frag =
                                    reinterpret_cast<EQEmu::S3D::WLDFragment2D&>(object_frags[frag_refs[m] - 1]);
                                auto m_ref = r_frag.GetData();

                                EQEmu::S3D::WLDFragment36& mod_frag =
                                    reinterpret_cast<EQEmu::S3D::WLDFragment36&>(object_frags[m_ref]);
                                auto mod = mod_frag.GetData();
                                placables.push_back(std::make_pair(plac, mod));
                            } else if(object_frags[frag_refs[m] - 1].type == 0x11) {
                                EQEmu::S3D::WLDFragment11& r_frag =
                                    reinterpret_cast<EQEmu::S3D::WLDFragment11&>(object_frags[frag_refs[m] - 1]);
                                auto s_ref = r_frag.GetData();

                                EQEmu::S3D::WLDFragment10& skeleton_frag =
                                    reinterpret_cast<EQEmu::S3D::WLDFragment10&>(object_frags[s_ref]);
