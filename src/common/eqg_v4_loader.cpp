...
bool EQEmu::EQG4Loader::Load(std::string file, std::shared_ptr<EQG::Terrain>& terrain) {
    ...
    if(!zon_found) {
        eqLogMessage(
            LogError, "Failed to open %s.eqg because the %s.zon file could not be found.", file.c_str(), file.c_str());
        return false;
    }
...
bool EQEmu::EQG4Loader::ParseZoneDat(eqemu::format::pfs_archive& archive, std::shared_ptr<EQG::Terrain>& terrain) {
    ...
    eqLogMessage(
        LogError, "Header info for %s.eqg v4 %d %d %d", filename.c_str(), header.unk000, header.unk004, header.unk008);

    SafeStringAllocParse(base_tile_texture);
    SafeVarAllocParse(uint32_t, tile_count);

    float zone_min_x = (float)terrain->GetOpts().min_lat * (float)terrain->GetOpts().quads_per_tile *
                       (float)terrain->GetOpts().units_per_vert;
    // float zone_max_x = (float)(terrain->GetOpts().max_lat + 1) * (float)terrain->GetOpts().quads_per_tile *
    // (float)terrain->GetOpts().units_per_vert;

    float zone_min_y = (float)terrain->GetOpts().min_lng * (float)terrain->GetOpts().quads_per_tile *
                       terrain->GetOpts().units_per_vert;
    // float zone_max_y = (float)(terrain->GetOpts().max_lng + 1) * (float)terrain->GetOpts().quads_per_tile *
    // terrain->GetOpts().units_per_vert;
    ...
    for(uint32_t i = 0; i < tile_count; ++i) {
        ...
        float tile_start_y = zone_min_y + (tile_lng - 100000 - terrain->GetOpts().min_lng) *
                                              terrain->GetOpts().units_per_vert * terrain->GetOpts().quads_per_tile;
        float tile_start_x = zone_min_x + (tile_lat - 100000 - terrain->GetOpts().min_lat) *
                                              terrain->GetOpts().units_per_vert * terrain->GetOpts().quads_per_tile;
        ...
        for(uint32_t j = 0; j < single_placeable_count; ++j) {
            ...
            if(adjusted_x < 0)
                adjusted_x =
                    adjusted_x +
                    (-(int)(adjusted_x / (terrain->GetOpts().units_per_vert * terrain->GetOpts().quads_per_tile)) + 1) *
                        (terrain->GetOpts().units_per_vert * terrain->GetOpts().quads_per_tile);
            else
            ...
            if(adjusted_y < 0)
                adjusted_y =
                    adjusted_y +
                    (-(int)(adjusted_y / (terrain->GetOpts().units_per_vert * terrain->GetOpts().quads_per_tile)) + 1) *
                        (terrain->GetOpts().units_per_vert * terrain->GetOpts().quads_per_tile);
            else
            ...
            glm::vec3 p1(row_number * terrain->GetOpts().units_per_vert,
                         (quad % terrain->GetOpts().quads_per_tile) * terrain->GetOpts().units_per_vert,
                         quad_vertex1Z);
            glm::vec3 p2(p1.x + terrain->GetOpts().units_per_vert, p1.y, quad_vertex2Z);
            glm::vec3 p3(
                p1.x + terrain->GetOpts().units_per_vert, p1.y + terrain->GetOpts().units_per_vert, quad_vertex3Z);
        ...
        for(uint32_t j = 0; j < tog_ref_count; ++j) {
            ...
            std::vector<std::byte> tog_buffer;
...
bool EQEmu::EQG4Loader::ParseInvwDat(eqemu::format::pfs_archive& archive, std::shared_ptr<EQG::Terrain>& terrain) {
    std::vector<std::byte> invw;
    if(!archive.get("invw.dat", invw)) {
        return false;
    }

    char* buf = (char*)&invw[0];
...
bool EQEmu::EQG4Loader::GetZon(std::string file, std::vector<std::byte>& buffer) {
    buffer.clear();
...
void EQEmu::EQG4Loader::ParseConfigFile(std::vector<std::byte>& buffer, std::vector<std::string>& tokens) {
    tokens.clear();
    std::string cur;
...
bool EQEmu::EQG4Loader::ParseZon(std::vector<std::byte>& buffer, EQG::Terrain::ZoneOptions& opts) {
    if(buffer.size() < 5)
        return false;
