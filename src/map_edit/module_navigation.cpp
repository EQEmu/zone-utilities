...
void ModuleNavigation::OnClick(int mouse_button,
    ...
    if(m_mode == ModeNavMeshGen && mouse_button == GLFW_MOUSE_BUTTON_LEFT && !io.KeyShift && collide_hit) {
        ...
        CalcPath();
    } else if(m_mode == ModeNavMeshConnections && mouse_button == GLFW_MOUSE_BUTTON_LEFT && !io.KeyShift &&
              collide_hit) {
        if(!m_conn_start_set) {
            ...
            switch(m_connection_area) {
                case NavigationAreaFlagNormal: flag = NavigationPolyFlagNormal; break;
                case NavigationAreaFlagWater: flag = NavigationPolyFlagWater; break;
                case NavigationAreaFlagLava: flag = NavigationPolyFlagLava; break;
                case NavigationAreaFlagZoneLine: flag = NavigationPolyFlagZoneLine; break;
                case NavigationAreaFlagPvP: flag = NavigationPolyFlagPvP; break;
                case NavigationAreaFlagSlime: flag = NavigationPolyFlagSlime; break;
                case NavigationAreaFlagIce: flag = NavigationPolyFlagIce; break;
                case NavigationAreaFlagVWater: flag = NavigationPolyFlagVWater; break;
                case NavigationAreaFlagGeneralArea: flag = NavigationPolyFlagGeneralArea; break;
                case NavigationAreaFlagPortal: flag = NavigationPolyFlagPortal; break;
                case NavigationAreaFlagPrefer: flag = NavigationPolyFlagPrefer; break;
                case NavigationAreaFlagDisabled:
                default: flag = NavigationPolyFlagDisabled;
            }

            AddMeshConnection(m_conn_start,
                              *collide_hit,
                              m_connection_radius,
                              m_connection_dir,
                              (unsigned char)m_connection_area,
                              flag);
        }
        ...
        if(nearest_index != -1 && sqrtf(nearest_dist) <= m_connection_rads[nearest_index]) {
...
void ModuleNavigation::DrawTestUI() {
    ...
    status =
        status || ImGui::SliderFloat("Cost: V Water", &m_path_costs[NavigationAreaFlagVWater], 0.1f, 10.0f, "%.1f");
    status =
        status || ImGui::SliderFloat("Cost: Teleport", &m_path_costs[NavigationAreaFlagPortal], 0.1f, 10.0f, "%.1f");
    status = status || ImGui::SliderFloat(
                           "Cost: General Area", &m_path_costs[NavigationAreaFlagGeneralArea], 0.1f, 10.0f, "%.1f");
...
void ModuleNavigation::DrawMeshConnectionUI() {
    ...
    const char* area_types[] = {
        "Normal", "Water", "Lava", "ZoneLine", "PVP", "Slime", "Ice", "V Water", "Generic Area", "Portal", "Prefer"};
...
void ModuleNavigation::BuildNavigationMesh() {
    ...
    for(int y = 0; y < th; ++y) {
        for(int x = 0; x < tw; ++x) {
            ...
            eqemu::event::task<bool> task([work](eqemu::event::task<bool>::success_fn success,
                                                 eqemu::event::task<bool>::failure_fn failure) { work->Run(); });
            ...
            task.run(eqemu::event::event_loop::get());
        }
    }
}
...
void ModuleNavigation::CreateChunkyTriMesh(std::shared_ptr<ZoneMap> zone_geo) {
    ...
    if(!rcCreateChunkyTriMesh((float*)zone_geo->GetCollidableVerts().data(),
                              (int*)inds.data(),
                              (int)inds.size() / 3,
                              512,
                              m_chunky_mesh.get())) {
...
void ModuleNavigation::SetNavigationTestNodeStart(const glm::vec3& p) {
    ...
    m_start_path_renderable->Clear();
    float sz = 2.0f;
    m_start_path_renderable->AddLineCylinder(
        glm::vec3(m_path_start.x - sz, m_path_start.y, m_path_start.z - sz),
...
dtStatus ModuleNavigation::GetPolyHeightNoConnections(const dtNavMeshQuery* query,
                                                      dtPolyRef ref,
                                                      const float* pos,
                                                      float* height) const {
...
dtStatus ModuleNavigation::GetPolyHeightOnPath(const dtPolyRef* path,
                                               const int path_len,
                                               const glm::vec3& pos,
...
void ModuleNavigation::CalcPath() {
    ...
    eqLogMessage(LogInfo,
                 "Calculating path from (%.2f, %.2f, %.2f) -> (%.2f, %.2f, %.2f)",
                 m_path_start[0],
                 m_path_start[1],
                 m_path_start[2],
                 m_path_end[0],
    ...
    if(m_complex_path) {
        if(npolys) {
            ...
            query->findStraightPath(&m_path_start[0],
                                    &m_path_end[0],
                                    polys,
                                    npolys,
                                    (float*)&spath[0],
            ...
            m_path_renderable->AddLineCylinder(glm::vec3(initial.x - 0.5f, initial.y - 0.5f, initial.z - 0.5f),
                                               glm::vec3(initial.x + 0.5f, initial.y + 0.5f, initial.z + 0.5f),
            ...
            for(auto i = 0; i < spath_n - 1; ++i) {
                ...
                if(flag & DT_STRAIGHTPATH_OFFMESH_CONNECTION) {
                    ...
                    while(total < dist) {
                        ...
                        eqLogMessage(LogInfo,
                                     "Adding current point (%.2f, %.2f, %.2f) %.2f/%.2f",
                                     current_pt.x,
                                     current_pt.y,
                                     current_pt.z,
                                     total,
                                     dist);
                        m_path_renderable->AddLineCylinder(
                            glm::vec3(current_pt.x - 0.5f, current_pt.y - 0.5f, current_pt.z - 0.5f),
                            glm::vec3(current_pt.x + 0.5f, current_pt.y + 0.5f, current_pt.z + 0.5f),
                            glm::vec3(1.0f, 1.0f, 0.0f));
        ...
        if(npolys) {
            ...
            if(spath_n) {
                ...
                for(auto i = 0; i < spath_n - 1; ++i) {
                    ...
                    auto& y2 = spath[i * 3 + 4];
                    auto& z2 = spath[i * 3 + 5];
                    m_path_renderable->AddLine(
                        glm::vec3(x1, y1, z1), glm::vec3(x2, y2, z2), glm::vec3(1.0f, 1.0f, 0.0f));
                }
...
void ModuleNavigation::SaveNavSettings() {
    ...
    // zone_name.navprj
    std::string filename =
        eqemu::core::config::instance().get_path("project", "maps/project/") + "/" + m_scene->GetZoneName() + ".navprj";
...
void ModuleNavigation::SaveNavMesh() {
    ...
    std::string filename =
        eqemu::core::config::instance().get_path("nav", "maps/nav/") + "/" + m_scene->GetZoneName() + ".nav";
    FILE* f = fopen(filename.c_str(), "wb");
    ...
    if(f) {
        ...
        uint32_t out_size = (uint32_t)eqemu::core::deflate_data(
            (const std::byte*)ss.str().c_str(), (uint32_t)ss.str().length(), (std::byte*)&buffer[0], buffer_len);
...
void ModuleNavigation::LoadNavMesh() {
    ...
    if(f) {
        ...
        uint32_t v =
            eqemu::core::inflate_data((const std::byte*)&data[0], data_size, (std::byte*)&buffer[0], buffer_size);
        fclose(f);
...
void ModuleNavigation::LoadVolumes() {
    ...
    for(auto& region : regions) {
        ...
        switch(region.type) {
            case RegionTypeNormal: v.area_type = NavigationAreaFlagNormal; break;
            case RegionTypeWater: v.area_type = NavigationAreaFlagWater; break;
            case RegionTypeLava: v.area_type = NavigationAreaFlagLava; break;
            case RegionTypePVP: v.area_type = NavigationAreaFlagPvP; break;
            case RegionTypeSlime: v.area_type = NavigationAreaFlagSlime; break;
            case RegionTypeIce: v.area_type = NavigationAreaFlagIce; break;
            case RegionTypeVWater: v.area_type = NavigationAreaFlagVWater; break;
            case RegionTypeGeneralArea: v.area_type = NavigationAreaFlagGeneralArea; break;
            case RegionTypePreferPathing: v.area_type = NavigationAreaFlagPrefer; break;
            case RegionTypeDisableNavMesh:
            default: v.area_type = NavigationAreaFlagDisabled;
        }
...
void ModuleNavigation::AddMeshConnection(const glm::vec3& start,
                                         const glm::vec3& end,
                                         float radius,
                                         unsigned char dir,
                                         unsigned char area,
...
void ModuleNavigation::UpdateConnectionsModel() {
    ...
    for(unsigned int i = 0; i < m_connection_count; ++i) {
        ...
        duAppendArc(&dd,
                    ...
                    v2[2],
                    0.25f,
                    (m_connection_dirs[i] & 1) ? 0.6f : 0.0f,
                    0.6f,
                    baseColor);
    }
    dd.end();
...
void NavigationDebugDraw::begin(duDebugDrawPrimitives prim, float size) {
    verts_in_use = 0;
    switch(prim) {
        case DU_DRAW_POINTS: mode = 1; break;
        case DU_DRAW_LINES: mode = 2; break;
        case DU_DRAW_TRIS: mode = 3; break;
        case DU_DRAW_QUADS: mode = 4; break;
    };
}
...
void NavigationDebugDraw::vertex(const float x,
                                 const float y,
                                 const float z,
                                 unsigned int color,
                                 const float u,
