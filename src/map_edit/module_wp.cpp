...
void ModuleWP::BuildVisualGraph(bool rebuild) {
    ...
    for(auto& node : m_nodes) {
        ...
        if(rebuild) {
            m_nodes_renderable->AddBox(glm::vec3(node.x - sz, node.z - sz, node.y - sz),
                                       glm::vec3(node.x + sz, node.z + sz, node.y + sz),
    ...
    for(auto& edge : m_edges) {
        ...
        if(edge.from == m_selected_node || edge.to == m_selected_node) {
            m_edges_renderable->AddLine(
                glm::vec3(from.x, from.z, from.y), glm::vec3(to.x, to.z, to.y), glm::vec3(1.0, 1.0, 0.0));
        } else {
            m_edges_renderable->AddLine(
                glm::vec3(from.x, from.z, from.y), glm::vec3(to.x, to.z, to.y), glm::vec3(1.0, 1.0, 1.0));
        }
    }
...
int32_t ModuleWP::GetSelectedNode(const glm::vec3& loc) {
    ...
    for(auto i = 0; i < m_nodes.size(); ++i) {
        auto& node = m_nodes[i];
        float d = eqemu::math::distance(glm::vec3(node.x, node.y, node.z), loc);
        if(d < distance) {
            current = i;
            distance = d;
        }
...
void ModuleWP::ConnectNodeToNode(int a, int b) {
    ...
    if(!connected) {
        ...
        if(!e.teleport) {
            e.distance = eqemu::math::distance(glm::vec3(n_a.x, n_a.y, n_a.z), glm::vec3(n_b.x, n_b.y, n_b.z));
        } else {
            e.distance = 0.0f;
        }
        e.from = a;
...
void ModuleWP::RecalcDistancesForNode(int id) {
    for(auto& edge : m_edges) {
        if(!edge.teleport && (edge.to == id || edge.from == id)) {
            ...
            edge.distance = eqemu::math::distance(glm::vec3(from.x, from.y, from.z), glm::vec3(to.x, to.y, to.z));
        }
    }
}
