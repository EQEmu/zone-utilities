...
const char* GetRegionTypeString(WaterRegionType type) {
    switch(type) {
        case RegionTypeNormal: return "Normal";
        case RegionTypeWater: return "Water";
        case RegionTypeLava: return "Lava";
        case RegionTypeZoneLine: return "ZoneLine";
        case RegionTypePVP: return "PvP";
        case RegionTypeSlime: return "Slime";
        case RegionTypeIce: return "Ice";
        case RegionTypeVWater: return "V Water";
        case RegionTypeGeneralArea: return "Misc";
        default: return "Unsupported";
    }
}

Scene::Scene() : m_camera_view(0), m_camera_proj(0), m_camera_loc(0) {
}
...
void Scene::Init(GLFWwindow* win) {
    ...
    const char* vertexProgramData = "#version 330 core\n"
    ...
    const char* fragmentProgramData = "#version 330 core\n"
                                      "in vec4 frag_color;\n"
                                      "out vec4 color;\n"
                                      "void main()\n"
                                      "{\n"
...
void Scene::LoadScene(const char* zone_name) {
    ...
    if(m_zone_geometry) {
        ...
        auto w_map = WaterMap::LoadWaterMapfile(eqemu::core::config::instance().get_path("volume", "maps/volume") + "/",
                                                zone_name);
        if(!w_map) {
            w_map = WaterMap::LoadWaterMapfile(eqemu::core::config::instance().get_path("water", "maps/water") + "/",
                                               zone_name);
        }
        m_physics->RegisterMesh("CollideWorldMesh",
                                m_zone_geometry->GetCollidableVerts(),
                                m_zone_geometry->GetCollidableInds(),
                                glm::vec3(0.0f, 0.0f, 0.0f),
                                EQPhysicsFlags::CollidableWorld);
        m_physics->RegisterMesh("NonCollideWorldMesh",
                                m_zone_geometry->GetNonCollidableVerts(),
                                m_zone_geometry->GetNonCollidableInds(),
                                glm::vec3(0.0f, 0.0f, 0.0f),
                                EQPhysicsFlags::NonCollidableWorld);
...
void Scene::RenderUI() {
    if(m_show_debug) {
        ...
        if(GetZoneGeometry()) {
            ...
            ImGui::Text("Min: (%.2f, %.2f, %.2f)",
                        zone_geo->GetCollidableMin().x,
                        zone_geo->GetCollidableMin().y,
                        zone_geo->GetCollidableMin().z);
            ImGui::Text("Max: (%.2f, %.2f, %.2f)",
                        zone_geo->GetCollidableMax().x,
                        zone_geo->GetCollidableMax().y,
                        zone_geo->GetCollidableMax().z);
        }
        ImGui::End();
    }
    ...
    if(m_show_options) {
        ...
        if(m_zone_geometry) {
            ...
            if(ImGui::DragFloatRange2("X",
                                      &m_bounding_box_min.x,
                                      &m_bounding_box_max.x,
                                      1.0f,
                                      m_zone_geometry->GetCollidableMin().x,
                                      m_zone_geometry->GetCollidableMax().x,
                                      "Min: %.1f",
                                      "Max: %.1f")) {
                update_bb = true;
            }

            if(ImGui::DragFloatRange2("Y",
                                      &m_bounding_box_min.y,
                                      &m_bounding_box_max.y,
                                      1.0f,
                                      m_zone_geometry->GetCollidableMin().y,
                                      m_zone_geometry->GetCollidableMax().y,
                                      "Min: %.1f",
                                      "Max: %.1f")) {
                update_bb = true;
            }

            if(ImGui::DragFloatRange2("Z",
                                      &m_bounding_box_min.z,
                                      &m_bounding_box_max.z,
                                      1.0f,
                                      m_zone_geometry->GetCollidableMin().z,
                                      m_zone_geometry->GetCollidableMax().z,
                                      "Min: %.1f",
                                      "Max: %.1f")) {
                update_bb = true;
            }
...
void Scene::ProcessSceneInput(float delta_time) {
    ...
    if(!io.WantCaptureKeyboard && (glfwGetKey(m_window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS ||
                                   glfwGetKey(m_window, GLFW_KEY_RIGHT_CONTROL) == GLFW_PRESS)) {
        move = false;
    }

    if(!io.WantCaptureKeyboard && (glfwGetKey(m_window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS ||
    ...
    if(click_to_process) {
        ...
        glm::vec3 collide_hit_loc(0.0);
        glm::vec3 hit_loc(0.0);
        std::string ent_name;
        ...
        if(had_hit) {
            ...
            if(ent_name.compare("CollideWorldMesh") == 0) {
                ...
                did_collide_hit =
                    m_physics->GetRaycastClosestHit(start, end, collide_hit_loc, nullptr, CollidableWorld);
            }
            ...
            for(int i = 0; i < 5; ++i) {
                ...
                for(auto& module : m_modules) {
                    if(module->GetRunning() && module->GetUnpaused()) {
                        module->OnClick(i,
                                        (did_collide_hit ? &collide_hit_loc : nullptr),
...
bool Scene::TryHotkey() {
    ...
    for(auto& hotkey : m_hotkeys) {
        if(!hotkey_hit && io.KeysDown[hotkey.key] == GLFW_RELEASE && m_key_status[hotkey.key] == GLFW_PRESS &&
           (!hotkey.modifiers || hotkey.modifiers & mods)) {
...
void Scene::OnHotkey(int ident) {
    switch(ident) {
        case MainHotkeyQuit: glfwSetWindowShouldClose(m_window, 1); break;
        case MainHotkeyOpen: m_show_open = true; break;
        case MainHotkeySave: {
            for(auto& module : m_modules) {
                if(module->GetRunning() && module->GetUnpaused() && module->CanSave()) {
                    module->Save();
        ...
        case MainHotkeyToggleOptions: m_show_options = !m_show_options; break;
        case MainHotkeyToggleDebug: m_show_debug = !m_show_debug; break;
    }
}
