...
void ModuleNavigationBuildTile::Run() {
    ...
    cfg.detailSampleDist =
        m_nav_module->m_detail_sample_dist < 0.9f ? 0 : m_nav_module->m_cell_size * m_nav_module->m_detail_sample_dist;
    ...
    for(int i = 0; i < ncid; ++i) {
        ...
        rcMarkWalkableTriangles(m_ctx.get(), cfg.walkableSlopeAngle, verts, nverts, ctris, nctris, triareas);
    ...
    for(auto& volume : m_nav_module->m_volumes) {
        rcMarkConvexPolyArea(
            m_ctx.get(), volume.verts, 4, volume.min, volume.max, (unsigned char)volume.area_type, *chf);
    }
    ...
    if(!rcBuildPolyMeshDetail(m_ctx.get(), *pmesh, *chf, cfg.detailSampleDist, cfg.detailSampleMaxError, *dmesh)) {
        rcFreeCompactHeightfield(chf);
        rcFreeContourSet(cset);
        rcFreePolyMesh(pmesh);
    ...
    if(cfg.maxVertsPerPoly <= DT_VERTS_PER_POLYGON) {
        ...
        for(int i = 0; i < pmesh->npolys; ++i) {
            ...
            switch(pmesh->areas[i]) {
                case NavigationAreaFlagNormal: pmesh->flags[i] = NavigationPolyFlagNormal; break;
                case NavigationAreaFlagWater: pmesh->flags[i] = NavigationPolyFlagWater; break;
                case NavigationAreaFlagLava: pmesh->flags[i] = NavigationPolyFlagLava; break;
                case NavigationAreaFlagZoneLine: pmesh->flags[i] = NavigationPolyFlagZoneLine; break;
                case NavigationAreaFlagPvP: pmesh->flags[i] = NavigationPolyFlagPvP; break;
                case NavigationAreaFlagSlime: pmesh->flags[i] = NavigationPolyFlagSlime; break;
                case NavigationAreaFlagIce: pmesh->flags[i] = NavigationPolyFlagIce; break;
                case NavigationAreaFlagVWater: pmesh->flags[i] = NavigationPolyFlagVWater; break;
                case NavigationAreaFlagGeneralArea: pmesh->flags[i] = NavigationPolyFlagGeneralArea; break;
                case NavigationAreaFlagPortal: pmesh->flags[i] = NavigationPolyFlagPortal; break;
                case NavigationAreaFlagPrefer: pmesh->flags[i] = NavigationPolyFlagPrefer; break;
                case NavigationAreaFlagDisabled:
                default: pmesh->flags[i] = NavigationPolyFlagDisabled;
            }
        }
        ...
        params.offMeshConVerts =
            m_nav_module->m_connection_count ? (float*)&m_nav_module->m_connection_verts[0] : nullptr;
        params.offMeshConRad = m_nav_module->m_connection_count ? (float*)&m_nav_module->m_connection_rads[0] : nullptr;
        params.offMeshConDir =
            m_nav_module->m_connection_count ? (unsigned char*)&m_nav_module->m_connection_dirs[0] : nullptr;
        params.offMeshConAreas =
            m_nav_module->m_connection_count ? (unsigned char*)&m_nav_module->m_connection_areas[0] : nullptr;
        params.offMeshConFlags =
            m_nav_module->m_connection_count ? (unsigned short*)&m_nav_module->m_connection_flags[0] : nullptr;
        params.offMeshConUserID =
            m_nav_module->m_connection_count ? (unsigned int*)&m_nav_module->m_connection_ids[0] : nullptr;
